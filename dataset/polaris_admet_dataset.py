...
def make_conversation(example, property):
    """Convert dataset examples into conversation format."""
    return {
        "ground_truth": example["solution"],
        "prompt": [
            {"role": "system", "content": SYSTEM_PROMPT(property)},
...
def load_polaris_dataset(params=["MLM", "HLM", "KSOL", "LogD", "MDR1-MDCKII"], rewrite=False):
    ...
    for split in dataset:
        dataset[split] = dataset[split].map(partial(make_conversation, property="LogD"))
